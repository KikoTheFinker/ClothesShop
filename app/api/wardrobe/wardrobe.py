...
@router.post("/create")
async def create_user_wardrobe(wardrobe: WardrobeCreate, db: Session = Depends(get_db),
...
@router.put("/update")
async def update_user_wardrobe(wardrobe: WardrobeUpdate, db: Session = Depends(get_db),
                               current_user=Depends(get_current_user)):
