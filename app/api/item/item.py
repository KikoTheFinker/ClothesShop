...
from app.services.item_service import create_item, get_item_by_id, get_filtered_items, delete_item, update_item, \
    get_all
...
async def get_approved_items(db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    return get_all(db, current_user.id)


@router.post("/user/wardrobe/items/add-item")
async def create_user_item(item: schemas.ItemCreate, db: Session = Depends(get_db),
...
async def update_user_item(update_data: schemas.ItemUpdate, db: Session = Depends(get_db),
                           current_user=Depends(get_current_user)):
